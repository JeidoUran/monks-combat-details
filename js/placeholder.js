    ...
    static init() {
        ...
        Hooks.on("renderCombatTracker", (app, html, data) => {
            ...
            html.find(".combatant").each((_, element) => {
                ...
                if (combatant && !combatant.getFlag("monks-combat-details", "placeholder")) {
                    $($('<a>')
                        .addClass('combatant-control')
                        .attr('data-tooltip', i18n("MonksCombatDetails.CreatePlaceholder"))
                        .html('<i class="fas fa-user-plus"></i>')
                        .on("click", () => {
                            const combatantData = combatant.toObject();
                            delete combatantData._id;
                            combatantData.name = combatant.name + " [Placeholder]";
                            combatantData.initiative = combatant.initiative ? combatant.initiative - 1 : null;
        
                            new PlaceholderCombatantConfig(new Combatant(combatantData)).render(true);
