...
export class CombatTurn {
  ...
  static init() {
    ...
    let combatNextTurn = async function (wrapped, ...args) {
      ...
      if (current?.actor?.hasPlayerOwner && game.user.isGM) {
        ...
        for (let user of game.users) {
          if (
            user.getFlag("monks-combat-details", "remember-previous") &&
            current?.actor.ownership[user.id] ==
    ...
    Hooks.on("deleteCombat", function (combat) {
      if (
        ...
        ChatMessage.create(
          { user: game.user, flavor: i18n("MonksCombatDetails.RoundEnd") },
          { roundmarker: true }
        );
      }
    ...
    Hooks.on("updateCombat", async function (combat, delta) {
      ...
      if (
        ...
        if (targets && targets.length > 0) {
          ...
          for (let id of targets) {
            ...
            if (token && !token.hidden && !MonksCombatDetails.isDefeated(token))
      ...
      if (setting("round-chatmessages") && combat && game.user.isTheGM) {
        if (combatStarted) {
          ChatMessage.create(
            { user: game.user, flavor: i18n("MonksCombatDetails.RoundStart") },
            { roundmarker: true }
          );
        ...
        $(
          ...
          $(this)
            ...
            .scrollTop(
              Math.max(
                this.offsetTop -
                  $("#combat-popout .combat-tracker-header").height() -
                  32,
                0
              )
            ); // - $(this).height()
  ...
  static clearShadows() {
    for (let shadow of Object.values(CombatTurn.shadows)) shadow.destroy();
    CombatTurn.shadows = {};
  }
  ...
  static doDisplayTurn(combatant) {
    if (setting("showcurrentup") && !game.user.isGM) {
      ...
      } else ui.notifications.warn(msg);
    }
  ...
  static checkCombatTurn(combat) {
    ...
    if (combat && combat.started) {
        ...
        if (skip) {
          for (let [i, t] of combat.turns.entries()) {
            if (
              i <= from ||
              t.defeated ||
              t.actor?.statuses.has(CONFIG.specialStatusEffects.DEFEATED)
            )
              continue;
            next = i;
            break;
      ...
      if (next == undefined || next >= combat.turns.length) next = findNext(-1);
...
Hooks.on("drawRegionLayer", function (layer) {
  layer.shadows = layer.addChildAt(
    new PIXI.Container(),
    ...
    for (let combat of game.combats) {
      if (combat.active && combat.turnsound) {
        if (combat.turnsound.effectiveVolume != undefined) {
          combat.turnsound.volume =
            volume * (combat.turnsound.effectiveVolume ?? 1);
        }
      }
    }
  }
