...
export let error = (...args) =>
  console.error("monks-combat-details | ", ...args);
  ...
  let nonLibWrapper = () => {
    ...
    eval(`${prop} = function (event) {
            return func.call(this, ${
              type != "OVERRIDE" ? "oldFunc.bind(this)," : ""
            } ...arguments);
        }`);
  };
...
export let getVolume = () => {
  ...
  else return game.settings.get("core", "globalAmbientVolume");
};
...
export class MonksCombatDetails {
  ...
  static canDo(setting) {
    ...
    if (
      MonksCombatDetails._rejectlist[setting] != undefined &&
      MonksCombatDetails._rejectlist[setting].includes(game.system.id)
    )
      return false;
    if (
      MonksCombatDetails._onlylist[setting] != undefined &&
      !MonksCombatDetails._onlylist[setting].includes(game.system.id)
    )
      return false;
    return true;
  }
  ...
  static init() {
    ...
    try {
      Object.defineProperty(User.prototype, "isTheGM", {
        get: function isTheGM() {
          return (
            this ==
            (game.users.find((u) => u.hasRole("GAMEMASTER") && u.active) ||
              game.users.find((u) => u.hasRole("ASSISTANT") && u.active))
          );
        },
      });
    } catch {}
    ...
    if (game.system.id == "dnd5e")
      ...
      MonksCombatDetails.xpchart = [
        50, 400, 600, 800, 1200, 1600, 2400, 3200, 4800, 6400, 9600, 12800,
        19200, 25600, 38400, 51200, 76800, 102400, 153600, 204800, 307200,
        409600, 614400, 819200, 1228800, 1638400, 2457600, 3276800, 4915200,
        6553600, 9830400,
      ];
    }
    ...
    MonksCombatDetails.crChallenge = [
      { text: "MonksCombatDetails.unknown", rating: "unknown" },
      {
        text:
          game.system.id == "pf2e"
            ? "MonksCombatDetails.trivial"
            : "MonksCombatDetails.easy",
        rating: "easy",
      },
      {
        text:
          game.system.id == "pf2e"
            ? "MonksCombatDetails.low"
            : "MonksCombatDetails.average",
        rating: "average",
      },
      {
        text:
          game.system.id == "pf2e"
            ? "MonksCombatDetails.moderate"
            : "MonksCombatDetails.challenging",
        rating: "challenging",
      },
      {
        text:
          game.system.id == "pf2e"
            ? "MonksCombatDetails.severe"
            : "MonksCombatDetails.hard",
        rating: "hard",
      },
      {
        text:
          game.system.id == "pf2e"
            ? "MonksCombatDetails.extreme"
            : "MonksCombatDetails.epic",
        rating: "epic",
      },
    ];
    ...
    if (setting("enable-placeholders") != "false") PlaceholderCombatant.init();
    ...
    patchFunc(
      ...
      async (wrapped, ...args) => {
        ...
        game.settings.set(
          ...
          "show-combat-cr-in-combat",
          formData.showCombatCR
        );
        game.settings.set(
          "monks-combat-details",
          "hide-defeated",
          formData.hideDefeated
        );
        game.settings.set(
          "monks-combat-details",
          "reroll-initiative",
          formData.rerollInitiative
        );
        if (formData.combatPlaylist != setting("combat-playlist")) {
          if (game.combats.active?.active && game.combats.active?.started) {
            if (setting("combat-playlist")) {
              ...
              let currentlyPlaying = ui.playlists._playingSounds
                .map((ps) => (ps.playing ? ps.uuid : null))
                .filter((p) => !!p);
            ...
            if (formData.combatPlaylist) {
              ...
              let lastPlaying = combat.getFlag(
                "monks-combat-details",
                "lastPlaying"
              );
              if (lastPlaying) {
                for (let playing of lastPlaying) {
                  let sound = await fromUuid(playing);
                  if (sound) sound.parent?.playSound(sound);
                }
              }
            }
          }
        ...
        game.settings.set(
          "monks-combat-details",
          "combat-playlist",
          formData.combatPlaylist
        );
        $("#combat-popout").toggleClass(
          "hide-defeated",
          formData.hideDefeated == true
        );
        return wrapped(...args);
      }
    );
    ...
    patchFunc(
      "CombatTracker.prototype._onResize",
      async function (wrapped, ...args) {
        if (this.popOut) {
          game.user.setFlag(
            "monks-combat-details",
    ...
    patchFunc(
      "Combat.prototype.startCombat",
      async function (wrapped, ...args) {
        if (
          setting("prevent-initiative") &&
          this.turns.find((c) => c.initiative == undefined) != undefined
        ) {
    ...
    patchFunc(
      ...
      async function (wrapped, ...args) {
        ...
        if (
          (setting("hide-until-turn") || setting("hide-enemies")) &&
          game.user.isGM
        ) {
          options = options || {};
          ...
          if (hiddenIds.length > 0) {
            await wrapped.call(
              this,
              hiddenIds,
              foundry.utils.mergeObject(foundry.utils.duplicate(options), {
                messageOptions: { rollMode: "selfroll" },
              })
            );
          }
        }
    ...
    if (game.settings.get("monks-combat-details", "prevent-token-removal")) {
      patchFunc(
        "TokenHUD.prototype._onClickControl",
        function (wrapped, ...args) {
          let [event] = args;
          const button = event.currentTarget;
          if (button.dataset.action == "combat" && this.object.inCombat) {
            ui.notifications.warn(
              i18n("MonksCombatDetails.PreventTokenMessage")
            );
    ...
    let isVisible = function () {
      ...
      if (
        (setting("hide-enemies") && !this.combat.started) ||
        (setting("hide-until-turn") &&
          this.combat.started &&
          foundry.utils.getProperty(
            this,
            "flags.monks-combat-details.reveal"
          ) !== true)
      ) {
    ...
    patchFunc(
      ...
      async function (wrapped, ...args) {
        if (this.popOut && args.length) args[0].resizable = true;
        return wrapped(...args);
      }
    );

    patchFunc(
      "Combat.prototype._sortCombatants",
      function (wrapped, ...args) {
        let combatants = args;
        if (
          combatants.length &&
          setting("order-initiative") &&
          !combatants[0].combat.started
        ) {
          if (!game.user.isGM) {
            let [a, b] = args;
  ...
  static async ready() {
    ...
    if (
      !setting("transfer-settings-client") &&
      game.modules.get("monks-little-details")?.active
    ) {
    ...
    if (setting("combat-alert") && $("#combat-notification").length === 0) {
      $("#sidebar-tabs a[data-tab='combat']").append(
        `<i id="combat-notification" class="notification-pip fas fa-exclamation-circle" style="display: none;"></i>`
      );
    }
    ...
    document
      .querySelector(":root")
      .style.setProperty(
        "--MonksCombatDetails-large-print-size",
        setting("large-print-size") + "px"
      );
  ...
  static async transferSettings() {
    ...
    let setSetting = async function (name) {
      let oldChange = game.settings.settings.get(
        `monks-combat-details.${name}`
      ).onChange;
      game.settings.settings.get(`monks-combat-details.${name}`).onChange =
        null;
      let value = swapFilename(
    ...
    for (let scene of game.scenes) {
      for (let token of scene.tokens) {
        if (
          foundry.utils.getProperty(
            token,
            "flags.monks-little-details.displayBarsCombat"
          )
        ) {
          await token.update({
            "flags.monks-combat-details.displayBarsCombat":
              foundry.utils.getProperty(
                token,
                "flags.monks-little-details.displayBarsCombat"
              ),
    ...
    ui.notifications.warn(
      "Monk's Combat Details has transfered over settings from Monk's Little Details, you will need to refresh your browser for some settings to take effect.",
      { permanent: true }
    );
  ...
  static async transferSettingsClient() {
    ...
    ui.notifications.warn(
      "Monk's Combat Details has transfered over your personal settings from Monk's Little Details, you will need to refresh your browser for some settings to take effect.",
      { permanent: true }
    );

    await game.settings.set(
      "monks-combat-details",
      "transfer-settings-client",
      true
    );
  }
  ...
  static repositionCombat(app) {
    ...
    if (typeof position === "string") {
      left = position.endsWith("left")
        ? 120
        : sidebar.offsetLeft - app.position.width;
  ...
  static getCR(combat) {
    ...
    if (game.system.id == "pf2e") {
      ...
      for (let combatant of combat.combatants) {
        if (combatant.actor != undefined && combatant.token != undefined) {
          if (combatant.actor.type == "hazard") {
            hazardLevels.push({
              level: parseInt(
                combatant.actor.system.details.level?.value ?? 1,
                10
              ),
            ...
            npcLevels.push(
              parseInt(combatant.actor.system.details?.level?.value ?? "1", 10)
            );
          }
        }
      }
      ...
      var calcAPL = 0;
      if (apl.count > 0) calcAPL = Math.round(apl.levels / apl.count);

      xp = game.pf2e.gm.calculateXP(
        calcAPL,
        apl.count,
        npcLevels,
        hazardLevels,
      ...
      var partyCR = MonksCombatDetails.xpchart.filter(
        (budget, index) => xp.ratingXP >= budget || index == 0
      );
        ...
        if (combatant.actor != undefined && combatant.token != undefined) {
          if (combatant.token.disposition == 1) {
            ...
            if (combatant.actor.system?.classes) {
              levels = Object.values(combatant.actor.system?.classes).reduce(
                (a, b) => {
            ...
            if (combatantxp == undefined) {
              ...
              combatantxp =
                MonksCombatDetails.xpchart[
                  Math.clamp(levels, 0, MonksCombatDetails.xpchart.length - 1)
                ];
            }
            xp += combatantxp || 0;
          }
      ...
      if (apl.count > 0)
        calcAPL =
          Math.round(apl.levels / apl.count) +
          (apl.count < 4 ? -1 : apl.count > 5 ? 1 : 0);

      //get the CR of any unfriendly/neutral
      let cr = Math.clamp(
        MonksCombatDetails.xpchart.findIndex((cr) => cr > xp) - 1,
        0,
        MonksCombatDetails.xpchart.length - 1
      );
  ...
  static checkPopout(combat, delta) {
    let combatCreated =
      combat && combat.started !== true && setting("popout-when") == "created";
    let combatStarted =
      combat &&
      combat.started === true &&
      setting("popout-when") == "starts" &&
      ((delta?.round === 1 && (delta.turn === 0 || delta.turn === undefined)) ||
        delta?.bypass ||
        delta == undefined);
    ...
    if (
      ((game.user.isGM && ["everyone", "gmonly"].includes(opencombat)) ||
        (!game.user.isGM &&
          ...
          game.settings.get("monks-combat-details", "popout-combat"))) &&
  ...
  static async showShadows(data) {
    fromUuid(data.uuid).then((token) => {
      if (
        ...
        CombatTurn.shadows[token.id] == undefined
      ) {
  ...
  static async spellChange(data) {
    if (game.user.isTheGM) {
      ...
      ChatMessage.create({
        user: game.user,
        content: `<i>${player?.name}</i> ${
          setting("prevent-combat-spells") == "both"
            ? "attempted to change"
            : "has changed"
        } prepared spells: <br/><br/> ${data.name} <br/><br/> For <b>${
          actor?.name
        }</b> while in combat.`,
        whisper: whisper,
      });
  ...
  static async AutoDefeated(hp, actor, token) {
    if (setting("auto-defeated") != "none" && game.user.isGM) {
      token =
        token ||
        game.combats?.viewed?.combatants.find((c) => c.actor?.id == actor.id)
          ?.token;
      if (
        hp != undefined &&
        (setting("auto-defeated").startsWith("all") || token?.disposition != 1)
      ) {
        let combatant = token?.combatant;

        //check to see if the combatant has been defeated
        let defeated = setting("auto-defeated").endsWith("negative")
          ? hp < 0
          : hp == 0;
...
Hooks.once("setup", () => {
  game.settings.settings.get("monks-combat-details.nextup-message").default =
    i18n("MonksCombatDetails.Next");
  game.settings.settings.get("monks-combat-details.turn-message").default =
    i18n("MonksCombatDetails.Turn");
});
...
Hooks.on("createCombat", function (data, delta) {
  ...
  if (
    game.user.isGM &&
    setting("switch-combat-tab") &&
    ui.sidebar.activeTab !== "combat"
  )
...
Hooks.on("deleteCombat", async function (combat) {
  ...
  if (
    ...
    game.settings.get("monks-combat-details", "close-combat-when-done")
  ) {
    const tabApp = ui["combat"];
    if (tabApp._popout != undefined) {
      ...
      MonksCombatDetails.closeTimer = setInterval(function () {
        ...
        const tabApp = ui["combat"];
        if (
          MonksCombatDetails.closeCount > 100 ||
...
Hooks.on("updateCombat", async function (combat, delta) {
  ...
  if (
    game.user.isTheGM &&
    combat.combatant?.getFlag("monks-combat-details", "placeholder")
  ) {
    let removeAfter = combat.combatant?.getFlag(
      "monks-combat-details",
      "removeAfter"
    );
    if (
      ...
      removeAfter <=
        combat.round -
          combat.combatant.getFlag("monks-combat-details", "removeStart")
    ) {
  ...
  if (
    ...
    let combatants = combat.combatants
      .filter((c) => !c.getFlag("monks-combat-details", "placeholder"))
      .map((c) => c.id);
  ...
  if (
    game.user.isTheGM &&
    setting("combat-playlist") &&
    combat.started &&
    delta.round == 1 &&
    delta.turn == 0
  ) {
    if (
      setting("show-combat-playlist") &&
      $("#combat .combat-playlist-row").length
    ) {
...
Hooks.on("renderCombatTracker", async (app, html, data) => {
  if (
    !MonksCombatDetails.tracker &&
    app.options.id == "combat" &&
    app.options.popOut
  ) {
    ...
    if (playlistId) {
      ...
      if (playlist) {
        playlist.sounds.forEach((sound) => {
          musicDropdown.append(
            $("<option>").attr("value", sound.id).html(sound.name)
          );
        });
      }
    }
  ...
  if (
    ...
    game.user.isGM &&
    data.combat &&
    (!data.combat.started || setting("show-combat-cr-in-combat")) &&
    setting("show-combat-cr") &&
    MonksCombatDetails.xpchart != undefined
  ) {
    //calculate CR
    let crdata = MonksCombatDetails.getCR(data.combat);

    if (
      $("#combat .encounter-cr-row", html).length == 0 &&
      data.combat.combatants.size > 0
    ) {
      ...
      if (game.system.id == "pf2e") {
        crChallenge =
          MonksCombatDetails.crChallenge[
            Math.clamp(crdata.cr, 1, MonksCombatDetails.crChallenge.length - 1)
          ];
        ...
        if (crdata.count > 0)
          crChallenge =
            MonksCombatDetails.crChallenge[
              Math.clamp(
                crdata.cr - crdata.apl,
                0,
                MonksCombatDetails.crChallenge.length - 1
              )
            ];
        epicness =
          crChallenge.rating == "unknown"
            ? 0
            : Math.clamp(crdata.cr - crdata.apl - 3, 0, 5);
  ...
  if (!game.user.isGM && data.combat && data.combat.started) {
    $(
      '.combat-control[data-control="previousTurn"],.combat-control[data-control="nextTurn"]:last',
      html
    ).css({ visibility: "hidden" });
  }

  $(".token-effects .token-effect", html).each((i, el) => {
    let effect = CONFIG.statusEffects.find(
      (e) => e.id === el.dataset.statusId || e.icon == el.getAttribute("src")
    );
    if (effect) {
...
Hooks.on("renderSettingsConfig", (app, html, data) => {
  ...
  let parent = $('input[name="monks-combat-details.next-sound"]', html).closest(
    ".form-group"
  );
  $('input[name="monks-combat-details.next-sound"]', html)
    .css({ "flex-basis": "unset", "flex-grow": 1 })
    .insertAfter($('input[name="monks-combat-details.play-next-sound"]', html));
  parent.remove();

  btn
    .clone(true)
    .insertAfter($('input[name="monks-combat-details.next-sound"]', html));

  parent = $('input[name="monks-combat-details.turn-sound"]', html).closest(
    ".form-group"
  );
  $('input[name="monks-combat-details.turn-sound"]', html)
    .css({ "flex-basis": "unset", "flex-grow": 1 })
    .insertAfter($('input[name="monks-combat-details.play-turn-sound"]', html));
  parent.remove();

  btn
    .clone(true)
    .insertAfter($('input[name="monks-combat-details.turn-sound"]', html));

  parent = $('input[name="monks-combat-details.round-sound"]', html).closest(
    ".form-group"
  );
  $('input[name="monks-combat-details.round-sound"]', html)
    .css({ "flex-basis": "unset", "flex-grow": 1 })
    .insertAfter(
      $('input[name="monks-combat-details.play-round-sound"]', html)
    );
  parent.remove();

  btn
    .clone(true)
    .insertAfter($('input[name="monks-combat-details.round-sound"]', html));

  //only show popout-combat if it's a player and it's available
  let opencombat = setting("opencombat");
  $('input[name="monks-combat-details.popout-combat"]', html)
    .closest(".form-group")
    .toggle(!game.user.isGM && ["everyone", "playeronly"].includes(opencombat));

  $("<div>")
    .addClass("form-group group-header")
    .html(i18n("MonksCombatDetails.CombatPreparation"))
    .insertBefore(
      $('[name="monks-combat-details.prevent-initiative"]').parents(
        "div.form-group:first"
      )
    );
  $("<div>")
    .addClass("form-group group-header")
    .html(i18n("MonksCombatDetails.CombatDetails"))
    .insertBefore(
      $('[name="monks-combat-details.clear-targets"]').parents(
        "div.form-group:first"
      )
    );
  $("<div>")
    .addClass("form-group group-header")
    .html(i18n("MonksCombatDetails.CombatTracker"))
    .insertBefore(
      $('[name="monks-combat-details.switch-combat-tab"]').parents(
        "div.form-group:first"
      )
    );
  $("<div>")
    .addClass("form-group group-header")
    .html(i18n("MonksCombatDetails.CombatBars"))
    .insertBefore(
      $('[name="monks-combat-details.add-combat-bars"]').parents(
        "div.form-group:first"
      )
    );
  $("<div>")
    .addClass("form-group group-header")
    .html(i18n("MonksCombatDetails.CombatTurn"))
    .insertBefore(
      $('[name="monks-combat-details.shownextup"]').parents(
        "div.form-group:first"
      )
    );
});
...
Hooks.on("preUpdateItem", function (document, data, options, userid) {
  if (
    ...
    document.type == "spell" &&
    document.actor &&
    foundry.utils.getProperty(data, "system.preparation") != undefined
  ) {
    ...
    if (document.actor.inCombat) {
      if (
        setting("prevent-combat-spells") == "prevent" ||
        setting("prevent-combat-spells") == "both"
      ) {
      ...
      if (
        setting("prevent-combat-spells") == "true" ||
        setting("prevent-combat-spells") == "both"
      ) {
        let name = `Spell Level: ${document.system.level}, ${document.name}`;
        MonksCombatDetails.emit("spellChange", {
          user: game.user.id,
  ...
  if (
    ...
    if (a) {
      ...
      if (exists != data.defeated) {
        await a.toggleStatusEffect(effect?.id ?? effect, {
          active: data.defeated,
        });
      }
    }
  }
...
Hooks.on("renderCombatTrackerConfig", (app, html, data) => {
  ...
  $("<div>")
    ...
    .append(
      $("<p>")
        .addClass("notes")
        .html(i18n("MonksCombatDetails.CombatPlaylistHint"))
    )
    .insertAfter(
      $('select[name="core.combatTheme"]', html).closest(".form-group")
    );
  ...
  $("<div>")
    ...
    .append(
      $("<p>")
        .addClass("notes")
        .html(i18n("MonksCombatDetails.CombatMusicHint"))
    )
    .insertAfter(
      $('select[name="combatPlaylist"]', html).closest(".form-group")
    );
...
Hooks.on("renderCombatTracker", (app, html, data) => {
  ...
  html.find(".combatant").each((_, element) => {
    ...
    if ($el.find(".my-custom-button").length === 0) {
      // Bouton de position
      const positionBtn = $("<a>")
        .addClass("combatant-control my-custom-button")
        .attr(
          "data-tooltip",
          i18n("MonksCombatDetails.SetCombatantPositionTitle")
        )
        .html('<i class="fa-solid fa-users-rays"></i>')
        .on("click", async () => {
          ...
          if (proceed) {
            let idx = game.combats.viewed.turns.findIndex(
              (c) => c.id == combatant.id
            );
      ...
      positionBtn.insertBefore($el.find(".token-effects"));

      // Bouton de ciblage
      const targetBtn = $("<a>")
        .addClass("combatant-control my-custom-button")
        .attr("data-tooltip", i18n("MonksCombatDetails.Target"))
        .html('<i class="fas fa-crosshairs"></i>')
        .on("click", () => {
          if (combatant?.token?._object) {
            const targeted = !combatant.token._object.isTargeted;
            combatant.token._object.setTarget(targeted, {
              releaseOthers: false,
            });
          }
        });

      targetBtn.insertBefore($el.find(".token-effects"));
    }
  });
});
  ...
  app.registerTileAction("monks-combat-details", "placeholder", {
      ...
      for (let entity of entities) {
        if (entity instanceof TokenDocument) {
          let combatant = combat.combatants.find(
            (c) => c.token?.id == entity.id
          ) || { actorId: entity.actor.id, tokenId: entity.id };
          let initiative = action.data.initiative
            ? await game.MonksActiveTiles.getValue(action.data.initiative, args)
            : null;
          let removeAfter = action.data.remove
            ? await game.MonksActiveTiles.getValue(action.data.remove, args)
            : null;
          let name = action.data.name
            ? await game.MonksActiveTiles.getValue(action.data.name, args)
            : null;

          PlaceholderCombatant.createPlaceholder({
            combatant,
            initiative,
            removeAfter,
            name,
    ...
    content: async (trigger, action) => {
      ...
      return `<span class="action-style">${
        ...
        !!action.data?.initiative
          ? ` at <span class="details-style">"${action.data?.initiative}"</span>`
          : ""
      }${
        !!action.data?.remove
          ? ` remove after <span class="value-style">&lt;${action.data?.remove}&gt;"</span> rounds`
          : ""
      }`;
    },
  });
